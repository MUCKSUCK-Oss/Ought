...
For the UI we went with a centered popup instead of a full page — light/dark toggle, and each workspace card gets tagged as Work or Entertainment. We leaned on AI for some of the early brainstorming around the friction concept, and for writing parts of the CSS — mainly the toggle animation and card layout — which we then went back through and tuned by hand.
